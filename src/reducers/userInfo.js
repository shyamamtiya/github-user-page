...
export default (state = { userInfo: {}, userRepos: [], filterRepos: [] }, action = {}) => {
    switch (action.type) {
        ...
        case 'FILTER':
            ...
            console.log("df",filterInfo)
            switch (filterInfo.type) {
                case 'REPO_TYPE':{
                   let filterRepos = hasKeyValue(state.userRepos, filterInfo.value.filter);
                    return {...state, filterRepos};
                }
                   
                case 'LANGUAGE_TYPE':{
                    let filterRepos = hasKeyValue(state.userRepos, {'language': filterInfo.value.title});
                    return {...state, filterRepos};
                }
